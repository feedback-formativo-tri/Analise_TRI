...
def gera_scatter_comp(df, vetor_itens, valor1, valor2, examinando_A="", examinando_B="", titulo=""):
    ...
    fig.add_annotation(
        ...
        text=f"{examinando_A}<br>Habilidade θ: {round(valor2['theta'], 2)} ({round(valor2['theta_normal'], 2)})"
             ...
             f"<br>Acerto ao acaso: {round(valor2['chute'], 4)}"
    ...
    fig.add_annotation(
        ...
        text=f"{examinando_B}<br>Habilidade θ: {round(valor1['theta'], 2)} ({round(valor1['theta_normal'], 2)})"
             ...
             f"<br>Acerto ao acaso: {round(valor1['chute'], 4)}"
...
estado = "PA"
items = [38, 3, 36, 5]
valor1 = {'theta': -1.25, 'probabilidade': 0.5, 'discriminacao': 1.56, 'chute': 0, 'theta_normal': normalize_habil(-1.25)}
valor2 = {'theta': 4, 'probabilidade': 0.5, 'discriminacao': 0.18, 'chute': 0.0025, 'theta_normal': normalize_habil(4)}
item1 = "Item 38"
item2 = "Item 36"
df = pd.read_csv(f'probabilidades/df_prob_3PL_LTM_{area_conhecimento}_{estado}.csv')
...
pio.write_image(scatter, f"graficos_python/grafico_3PL_LTM_{area_conhecimento}_{estado}.pdf", format="pdf")
